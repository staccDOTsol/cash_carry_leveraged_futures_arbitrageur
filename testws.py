...
async def handle_ws_data(*args, **kwargs):
    ...
    if 'tick' in args[0]:
        ...
        if 'close' in d:
            #print(d)
            coin = args[0]['ch'].split('_')[0].split('.')[1]
            #print(coin)
            mids['futs'][coin] = float(d['close'])
    if 'data' in args[0]:
        for d in args[0]['data']:
            if 'symbol' in d:
                print(d['symbol'])
                print(d['margin_balance'])
...
if __name__ == "__main__":
    ...
    for coin in coinsto:
        market_subs.append({
                    "sub": "market." + coin + "_NQ.detail",
                    "id": str(uuid.uuid1())
                })
        market_subs.append({
                    "sub": "market. " + coin + "_CQ.detail",
                    "id": str(uuid.uuid1())
                })
        market_subs.append({
                    "sub": "market." + coin + "_NW.detail",
                    "id": str(uuid.uuid1())
                })
        market_subs.append({
                    "sub": "market." + coin + "_CW.detail",
                    "id": str(uuid.uuid1())
                })
       # order_subs.append({
       #             "op": "sub",
       #             "cid": str(uuid.uuid1()),
       #             "topic": "positions." + coin
       #         }
       #         )
        order_subs.append({
                    "op": "sub",
    ...
    while True: 
        try:
            ...
            loop.create_task(subscribe(market_url, access_key,  secret_key, market_subs, handle_ws_data, auth=False))
            loop.create_task(subscribe(order_url, access_key,  secret_key, order_subs, handle_ws_data, auth=True))

            #loop.create_task(loop_in_thread())
            loop.run_forever()
